...
% Original author: Philip Liu; AM160528: minimal formatting changes (annotated below)
...
figure 
    ...
    while i <= length(TDEEP);
        ...
        xlim([xmin,xmax])
        ylim([ymin,ymax])
        title(paraname(TDEEP(i),:), 'fontsize', 16); %AM160528: Fontsize reduced from 18 to 16
        i=i+1;
    end
...
figure 
    i=1;
    while i <= length(TEXO);
        subplot(5,4,i,'align');  %AM160528: Subplot dimesions changed from (5,4,i) to (4,5,i) (for better visibility)
        ...
        xlim([xmin,xmax])
        ylim([ymin,ymax])
        title(paraname(TEXO(i),:), 'fontsize', 16); %AM160528: Fontsize reduced from 18 to 16
        i=i+1;
    end
