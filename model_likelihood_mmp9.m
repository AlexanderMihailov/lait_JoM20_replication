function [ L, PROBLEM1,PROBLEM2,PROBLEM3 ] = model_likelihood_mmp9(Theta,dy,ny,nx,POLICY,nlp)

% model_likelihood_mmp9.m <=> AM190604 -> mmp9 version
%
% Last modified: AM190604 -> mmp9 version; AM150804; AM150730: CORRECTION in line 121, WITH NOW THE NEW PROXY FOR d(t) REPLACING
% consumption as the ADDED OBSERVABLE to the 9 observables in KLL!
%
...
% variance covariance
Omega = diag(Theta(29:37)); %AM190615 I changed the indexing, something was not working with nz or end?
%Omega = diag(Theta(end-nz:end-1)); %SHOULD BE THE SAME AS ABOVE - BUT DOESN'T WORK?!...
% AM190615 end-nz+1 = 38-9+1=30 end = 38 => from element 30 through element 38 = 9 elements selected
%                  end-nz-1 = 38-9-1=28 end-1 = 38-1 = 37 (as chi is Theta(38), in-between the epsilons/sigs)!!!
...
[PP,QQ,PROBLEM1] = model_solve_mmp9(Theta,POLICY);
%AM190604 added _mmp9 in the function name above
...
R= diag(Theta(29:37)); %AM190615 I changed the indexing, something was not working with nz or end?% POLICY=0, state vector = x(t)=[lambda(t) yt pit qt pict gt ut rst it]
...
% AM150730: SELECTING THE OBSERVABLE VARIABLES, WITH NOW ADDED PROXY FOR d(t)!
G(1,3+nlm) = 4;           % pi_F == 3
G(2,4+nlm) = 1;           % q
G(3,5+nlm) = 1;           % s
G(4,6+nlm) = 1;           % y
...
G(8,16+nlm) = 1;          % y*
G(9,17+nlm) = 4;          % r*
%G(10,22+nlm) = 1;         % Consumption == 1 Modified by Antonio July 2015; AM150730: d(t) == 22; c(t)==11 NOT 1 (1 is NOW ner, not used as observable, in the new IFS dataset)
