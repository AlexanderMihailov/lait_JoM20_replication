%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% bra_est_qifspq_099_020_005_mh0275_rng6twister_Gpv05_mmp9_muq0.m
% Last modified: AM190604/0611/0809 (muq=0) -> mmp9 version; AM160725 - SEE genTheta_bra099_020_005_mmp9_muq0.m in the "Functions calls"
% subsection below for the PARTICULAR calibration used (as "embedded" in the appended part of the function name:
% NEEDS TO BE CHANGED FOR ANY ALTERNATIVE CALIBRATION!)
% 
% This program estimates an extended version of the KLL (2009: JMCB) model
% including a preference shock and incomplete asset markets,
% for the Brazilian IMF/IFS (and OECD/National Accounts) quarterly data.
%
...
% Final modification and check (plus uniform formatting: max ~110 columns): Alexander Mihailov, 25 July 2016;
                                                                                          % 5=>11 June 2019 -> mmp9 version
% Tweaks by Antonio Pompa Rangel, July 7, 2019
%       lines:
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Script calls:
%     
% Function calls:
%     bra_datadoc_qifspq.m to manipulate and plot data series
%AM150805: added next 
%     genTheta_bra099_020_005_mmp9_muq0.m - fixes the 3 calibrated parameters, beta,
%          alpha and chi, respectively, for Brazil as indicated by the filename
%          NEEDS TO BE CHANGED FOR ANY ALTERNATIVE CALIBRATION!
%     model_likelihood_mmp9.m for evaluating the likelihood
...
warning off
delete output.txt
diary('bra_est_qifspq_099_020_005_mh0275_rng6twister_Gpv05_mmp9_output.txt') 
     %AM150809: country- and calibration-specific output file name 
disp('Estimating for Brazil: NK-foreign VAR model & muq not zero')
...
mh_scale = 0.275;    % scale variance to obtain optimal acceptance rate (20%)
...
ny = 22;            % number of state variables, rows(ynames) AM190604 1 less since epsilon_g deleted
         %(+2 NOW with INcomplete markets: foreign bond to SS-ouput d_t and preference shock g_t ADDED)
...
rng(6,'twister'); % AM151224: here the seed and the PRNG function may be changed for EACH run
...
disp(['rng (6,twister)']); % AM160113 addition to print out in output file

%% Import COUNTRY data and setting data frequency and estimation startdate
[data,datadate,raw] = xlsread('data/bradata_qifspq.xlsx'); % AM150730: NEW IFS-based data set including a proxy for d(t)
...
sampldate = 1999;
startdate = 2004; % AM190809 Brazil

% Call function to manipulate and plot data
[newdata,dtitle,date_vec] = bra_datadoc_qifspq(COUNTRY,data,datadate,interval,sampldate,startdate,PLOT_DATA); % APR07042019: had trouble plotting data
newdata(:,1)=[];            % get rid of the NER [AM150729: not consumption, as in KLL] series
newdata(:,10)=[];           % get rid of the consumption series, to use INSTEAD the added international reserves [AM150730] series
newdata(:,10)=[];           % APR07042019: get rid of d_t, as only 9 variables are needed.

% AM150805 THE NEXT BLOCK, CANNOT BE DELETED: while it seems repetitive (with regard to similar commands in
% bra_datadoc_qifspq, THE REDUCTION BY 3 COLUMNS JUST ABOVE of newdata AFFECTS its SIZE, AND HENCE
% THE RE-NUMBERING of the variables by the index i!

percent_index = [2,3,4,8]; %,10]; APR07042019: getting rid of 10 as it is not needed. % AM150729-IMPORTANT! 10 added - was MISSING (and the other index numbers cross-checked)!
...
para_latex_mmp9;                 % Create matrix of paramater names - text/LaTeX strings % AM190604 -> mmp9 version
                            % -> AM160518: comes from para_latex.m, a separate file that needs to be created
...
pri=[   0       0.99  0.001;  % AM160528 -> added Theta vector numbering: Theta(1); beta ~ calibrated
        0       0.4432335207   0.1;   % Theta(2); alpha ~ calibrated to the country-specific mean
        ...
        0       0.5   0.3;    % Theta(26) but (15) estimated; mu_q ~ GAMMA %check genTheta_[...].m, set newTheta(26)=0 if mu_q=0
        ...
        0       0.05  0.004;] ; % Theta(38) but NOT estimated; chi ~ calibrated to the value of JP(2010) in JAEtrics--->Added by Antonio 2014
                              % AM150805 MODIFIED VALUES AND CORRECTED genTheta.m - WAS WRONG BEFORE!
        %1       0.5   0.2;    % Theta(39) but (27) estimated; rhog ~ BETA--->Added by Antonio July 2015 (preference shock)
        %4       1     0.4;]   % Theta(40) but (28) estimated; sigmag ~ INVGAMMA --->Added by Antonio July 2015
        % AM190604: the last two parameters commented out for the mmp9 version here
...
%primeanTheta = pri(:,2); %AM160602: commented in %AM160529 commented out: works much better with the initialisation used by KLL in their mh_init0
...
%mh_init = (primeanTheta)'; %AM160602: commented in

%mh_init = ones(1,npara); %AM190605 added; before doing that got half a day error message mh_init not defined...

load mh_init0

mh_init = mh_init0; %AM190605 added, alternatively to above; both then commented out, the error message is replaced by another:
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%"Error using  * 
%Inner matrix dimensions must agree.

%Error in model_likelihood_mmp9 (line 97)
%C=QQ*Omega;

%Error in bra_est_qifspq_099_020_005_mh0275_rng6twister_Gpv05_mmp9 (line 364)
%[loglike,PROBLEM1,PROBLEM2,PROBLEM3] = ... "
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% AM190605 The problem comes, I guess, from the different number of parameters in the
% mh_init0 vector, 37 in kll, 40 in our 10x10 model and 38 now in this mmp9 (9x9) vesrion.
% It was always tricky using this KLL code to deal with such modifications in the mh_intl0.mat
% file that we create - it seems it carries over reproducing old versions or interfereing from somewhere,
% but I can't figure out where it comes from an dhow to amned it in th emany files of codes and updates
% and variants. Any ideas, Antonio, Steve?
% If I leave it like it was in the old codes, and here above, I get the following error message:
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%"Undefined function or variable 'mh_init'.
%
%Error in bra_est_qifspq_099_020_005_mh0275_rng6twister_Gpv05_mmp9 (line 263)
%    Theta = mh_init "
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%
%!!! AM190605 This initial *.mat file needs to be first created (using creating_mat_vector.m code of today) and available in the folder; 0 deleted from the name mh_init0, then restored
% AM160527 (commented out): load mh_init0 %AM160529 commented in (above): works much better with KLL'S mh_init0
...
if POLICY == 0,
    ...
    variance_normal(11)=0.01; %modified by Antonio
    ...
    variance_normal(38)=0.05;    %AM160518: chi variance changed from 0.05 to 0.01. Modified back by Antonio
    %variance_normal(39)=0.05;    %Antonio July 2015 => AM190605 Commented out (after almost half-wasted day of error messages) for mmp9 version
    %variance_normal(40)=0.05;    %Antonio July 2015 => AM190605 same comment as just above (finally!!!)
...
%     variance_normal(42)=0.05;
    ...
    variance_normal(36)=0.05;
    variance_normal(37)=0.05;
    variance_normal(38)=0.01;   %AM190605: chi variance now also changed from 0.05 to 0.01 (as it was much earlier in the discretion case above)
   % variance_normal(39)=0.05;   %Antonio July 2015 => Alex 190605 commented out for mmp9 version
   % variance_normal(40)=0.05;   %Antonio July 2015 => Alex 190605 same comment as just above
%     variance_normal(41)=0.05;
%     variance_normal(42)=0.05;
end
...
[loglike,PROBLEM1,PROBLEM2,PROBLEM3] = ...
        model_likelihood_mmp9(Theta,newdata,ny,nx,POLICY,NLAGPOLICY);
% AM190604 -> mmp9 version (above in name, appended)
...
save initiallikepriNK_bra_est_qifspq_099_020_005_mh0275_rng6twister_Gpv05_mmp9 loglikelogpri %AM160222 %AM190604 _mmp9

pau=0;      % initial value for the acceptance rate, pau is acceptance rate 
test1=0;
probs1=0;
probs2=0;
hh = waitbar(0,'Starting RW Metropolis-Hastings'); %AM190605 "Hasting" corrected to "Hastings" (inconsequential for the code and results, of course)
...
for j=1:N,
        ...
        [newTheta] = genTheta_bra099_020_005_mmp9_muq0(Theta, 0, variance_normal, npara, bounds); %generate new Theta
                                  %AM190604 _mmp9  
        newlogpri = priorln(newTheta, pshape, p1, p2);
        
        if newlogpri>-Inf 
                [newloglike,PROBLEM1,PROBLEM2,PROBLEM3] = ...
                        model_likelihood_mmp9(newTheta,newdata,ny,nx,POLICY,NLAGPOLICY);
                                   %AM190604 _mmp9
...
%    end
    ...
    toc;
    disp(['completion rate (%): ', num2str(prtfrc*100)]);              % AM160204: all 3 running statistics here harmonised as %
    disp(['acceptance rate (%): ', num2str(pau/j*100)]);               % AM160204: all 3 running statistics here harmonised as %
...
fid4=fopen('bra_est_qifspq_099_020_005_mh0275_rng6twister_Gpv05_mmp9_NKsummary.txt','w+'); %AM190604 _mmp9
...
Thetapost01TPOL = Thetapost(:,15:16);   %AM160528: Picking up the policy parameters (numbering as estimated - see above) AM190806 mu_q=0 (estimated 17) excluded 
...
%Thetapost02Trhog = Thetapost(:,26);     %AM160528: Picking up the
%persistence parameter for the preference shock g_t %AM190604 _mmp9
Thetapost02Tsigma = Thetapost(:,17:25); %AM160528: Picking up the st.dev. parameters (numbering as estimated) !!! AM190803 changed (after error message) from 18:26 to 17:25 in muq=0
%Thetapost02Tsigmag = Thetapost(:,27);   %AM160528: Picking up the
% st.dev. parameter for the preference shock g_t %AM190604 _mmp9
Thetapost02 = horzcat(Thetapost02Trho,Thetapost02Tsigma); %AM160528: concatenating
%AM190604 Thetapost02Trhog,Thetapost02Tsigmag taken out of the brackets for this mmp9 version

paran01TPOL = paran(15:16,:);  %AM160528: Picking up the LABELS for the policy parameters (numbering as estimated) AM190806 17 changed to 16 to exclude mu_q
...
%paran02Trhog = paran(26,:);   %AM160528: Picking up the LABELS for the persistence parameters (numbering as estimated)
%AM190604 _mmp9
paran02Tsigma = paran(17:25,:);%AM160528: Picking up the LABELS for the st.dev. parameters (numbering as estimated) AM190803 26 replecaed by 25 after error message in muq=0 version
%paran02Tsigmag = paran(27,:);%AM160528: Picking up the LABELS for the st.dev. parameters (numbering as estimated)
%AM190604 _mmp9
paran02 = vertcat(paran02Trho,paran02Tsigma); %AM160528: concatenating the above 4 matrices 
%AM190604 taken out of the brackets in this mmp9 version paran02Trhog,paran02Tsigmag
...
matrix2latex2(priormat, 'bra_est_qifspq_099_020_005_BRAprior.tex', 'rowLabels', rowlabel, ...
            'columnLabels', collabel, 'alignment', 'c', 'format', '%-6.3f','size', 'tiny'); %AM160305 format 3 deicmals    

collabel = {'Post Mean','Post SD','2.5\%','97.5\%'};        
matrix2latex2(newstatmat, 'bra_est_qifspq_099_020_005_mh0275_rng6twister_Gpv05_mmp9_BRAstat.tex', 'rowLabels', rowlabel, ...
            'columnLabels', collabel, 'alignment', 'c', 'format', '%-6.3f','size', 'tiny'); 
        
% AM160222: adding a table for convmat
collabel = {'NSE(8\%)','p-value','B-G'};        
matrix2latex2(convmat, 'bra_est_qifspq_099_020_005_mh0275_rng6twister_Gpv05_mmp9_BRAconv.tex', 'rowLabels', rowlabel, ...
            'columnLabels', collabel, 'alignment', 'c', 'format', '%-6.3f','size', 'tiny');
...
collabel = {'Post Mean','Post SD','2.5\%','97.5\%','NSE(8\%)','p-value','B-G'};        
matrix2latex2(statconvmat, 'bra_est_qifspq_099_020_005_mh0275_rng6twister_Gpv05_mmp9_BRAstatconv.tex', 'rowLabels', rowlabel, ...
            'columnLabels', collabel, 'alignment', 'c', 'format', '%-6.3f','size', 'tiny');
...
disp('DONE estimating for Brazil: NK-foreign VAR model & muq not zero')