%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% col_est_qifspq_099_029_mh04_rng3twister_kll_muq0.m   % _kll_muq0 appended to name, AM160518 
% Last modified: AM160707 - SEE genTheta_col099_029_kll_muq0.m in the "Functions calls" subsection below
...
% 
% This program estimates the original version of the KLL (2009: JMCB) model NOW with mu_q=0 (NO RER volatility target)
...
% for the Colombian IMF/IFS (and OECD/National Accounts) quarterly data.
%
...
% Final modification and check (plus uniform formatting: max ~110 columns): Alexander Mihailov, 7 July 2016
%
...
%AM150805: added next 
%     genTheta_col099_029_kll_muq0.m - fixes the 2 calibrated parameters, beta
%          and alpha, respectively, for Colombia as indicated by the filename
%          NEEDS TO BE CHANGED FOR ANY ALTERNATIVE CALIBRATION!
...
warning off
delete output.txt
diary('col_est_qifspq_099_029_mh04_rng3twister_kll_muq0_output.txt') 
     %AM150809: country- and calibration-specific output file name 
disp('Estimating for Colombia_kll_muq0: NK-foreign VAR model & muq NOW zero')
...
mh_scale = 0.4;     % scale variance to obtain optimal acceptance rate (20%)
...
sampldate = 1999; % AM150810: 2003 rather than 1999 here, to ignore columns with missing data, does NOT work
                            % since the raw matrix has different dimensions
startdate = 2003; % AM150808: no earlier data for some series

% Call function to manipulate and plot data
[newdata,dtitle,date_vec] = col_datadoc_qifspq(COUNTRY,data,datadate,interval,sampldate,startdate,PLOT_DATA);
...
pri=[   0       0.99  0.001;  % AM160528 -> added Theta vector numbering: Theta(1); beta ~ calibrated
        ...
        0       0.5   0.3;    % Theta(26)[0 REPLACED 2 in 1st column: AM160704; AND] NOW NOT (15) estimated; mu_q ~ GAMMA %check genTheta_[...].m, set newTheta(26)=0 if mu_q=0
        2       0.5   0.3;    % Theta(27) but (15) estimated; mu_y ~ GAMMA
        2       0.5   0.3;    % Theta(28) but (16) estimated; mu_r ~ GAMMA
        4       0.5   0.25;   % Theta(29) but (17) estimated; sigmah ~ INVGAMMA
        4       0.5   0.25;   % Theta(30) but (18) estimated; sigmaf ~ INVGAMMA
        4       1     0.4;    % Theta(31) but (19) estimated; sigmaa ~ INVGAMMA
        4       2     0.5;    % Theta(32) but (20) estimated; sigmaq ~ INVGAMMA
        4       1     0.4;    % Theta(33) but (21) estimated; sigmas ~ INVGAMMA
        4       1     0.4;    % Theta(34) but (22) estimated; sigmapi* ~ INVGAMMA
        4       1     0.4;    % Theta(35) but (23) estimated; sigmay* ~ INVGAMMA
        4       1     0.4;    % Theta(36) but (24) estimated; sigmar* ~ INVGAMMA
        4       1     0.4;]   % Theta(37) but (25) estimated; sigmar ~ INVGAMMA
...
TPOL = [15:16]; % pick index of policy parameters AM160528: numbering as the ESTIMATED parameters, NOT Theta(i) (see above)!
...
save initiallikepriNK_col_est_qifspq_099_029_mh04_rng3twister_kll_muq0 loglikelogpri %AM160222
...
for j=1:N,
        ...
        [newTheta] = genTheta_col099_029_kll_muq0(Theta, 0, variance_normal, npara, bounds);    %generate new Theta
...
fid4=fopen('col_est_qifspq_099_029_mh04_rng3twister_kll_muq0_NKsummary.txt','w+');
...
Thetapost01TPOL = Thetapost(:,15:16);   %AM160528: Picking up the policy parameters (numbering as estimated - see above)
...
Thetapost02Tsigma = Thetapost(:,17:25); %AM160528: Picking up the st.dev. parameters (numbering as estimated)
Thetapost02 = horzcat(Thetapost02Trho,Thetapost02Tsigma);  %AM160528: concatenating horizontally the above 2 matrices

paran01TPOL = paran(15:16,:);  %AM160528: Picking up the LABELS for the policy parameters (numbering as estimated)
...
paran02Tsigma = paran(17:25,:);%AM160528: Picking up the LABELS for the st.dev. parameters (numbering as estimated)
...
matrix2latex2(priormat, 'col_est_qifspq_099_029_kll_muq0_COLprior.tex', 'rowLabels', rowlabel, ...
            'columnLabels', collabel, 'alignment', 'c', 'format', '%-6.2f','size', 'tiny');

collabel = {'Post Mean','Post SD','2.5%','97.5%'};        
matrix2latex2(newstatmat, 'col_est_qifspq_099_029_mh04_rng3twister_kll_muq0_COLstat.tex', 'rowLabels', rowlabel, ...
            'columnLabels', collabel, 'alignment', 'c', 'format', '%-6.2f','size', 'tiny'); 
        
% AM160222: adding a table for convmat
collabel = {'NSE(8%)','p-value','B-G'};        
matrix2latex2(convmat, 'col_est_qifspq_099_029_mh04_rng3twister_kll_muq0_COLconv.tex', 'rowLabels', rowlabel, ...
            'columnLabels', collabel, 'alignment', 'c', 'format', '%-6.2f','size', 'tiny');
...
matrix2latex2(statconvmat, 'col_est_qifspq_099_029_mh04_rng3twister_kll_muq0_COLstatconv.tex', 'rowLabels', rowlabel, ...
            'columnLabels', collabel, 'alignment', 'c', 'format', '%-6.2f','size', 'tiny');
...
disp('DONE estimating for Colombia_kll_muq0: NK-foreign VAR model & muq not zero')